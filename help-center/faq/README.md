# Frequently asked questions

JoyProxy maintains an expanded FAQ on the main website with searchable categories. Use it for quick answers; use this documentation for step-by-step guides.

## Browse the live FAQ

**[joyproxy.com/faq.html](https://www.joyproxy.com/faq.html)**

Topics typically include:

- **Account** — registration, login, credits, security
- **Billing** — PayPal, balance, refunds, invoices
- **Proxy products** — rotating vs static vs custom, traffic, endpoints
- **Web Scraping API** — credits, tokens, fetch parameters
- **Troubleshooting** — auth errors, geo mismatches, connectivity

The FAQ updates with the product—when in doubt, prefer the live page over older screenshots.

## Still need help?

| Channel | When |
| --- | --- |
| [AI Assistant](https://www.joyproxy.com/products/ai-customer-service.html) | Instant answers 24/7 |
| [Live chat](../support/live-chat.md) | Real-time agent |
| [Ticket](../support/tickets.md) | Billing paper trail or complex bugs |

## Documentation paths

- New to JoyProxy → [Overview](../README.md)
- Ready to buy → [Purchase](https://www.joyproxy.com/admin-purchase.html) + [Pricing](../support/proxy-pricing.md)
- Integrating today → [Rotating](../getting-started/rotating-proxy.md) or [Web Scraping API](../getting-started/web-scraping-api.md)

A full Markdown import of FAQ entries into this repository is planned for a later release.
