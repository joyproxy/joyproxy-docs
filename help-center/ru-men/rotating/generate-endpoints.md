...
| 端口 | `9001`                                                    |
| 协议 | HTTP 与 SOCKS5。任一协议均可打开 **HTTPS 网站**。见 [协议](protocols.md)。 |

## 网页生成器

1. 打开[端点生成/提取中心](https://www.joyproxy.com/admin-ip-extraction-center.html)或网络控制台的 **Endpoints** 页签。
2. 选择 **Rotating** 与已付款的订单/网络（住宅、移动或商业）。
3. **Output format** — Endpoint:port、HTTP 或 SOCKS5。主机与端口保持 `gate.joyproxy.com:9001`。详情：[协议](protocols.md)。
4. **Location** — 国家，可选州/省与城市。见 [地域定向](location-targeting.md)。
5. **Session type** — **Sticky session**（1–30 分钟）或 **Rotating session**。见 [粘性会话](sticky-sessions.md)。
6. 设置生成行数 → **Generate**。
...
* 定向国家或城市：[地域定向](location-targeting.md)
* 登录流程保持同一 IP：[粘性会话](sticky-sessions.md)
* 验证线路：[首次请求](first-request.md)
