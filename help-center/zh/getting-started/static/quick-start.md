...
1. 进入 **<a href="https://www.joyproxy.com/admin-ip-extraction-center.html" target="_blank" rel="noopener noreferrer">提取</a>** 页面，切到 **静态代理** 页签。
2. 筛选你购买的订单线路，在 **输出格式** 选择展示样式。
3. 点击 **复制地址列表**，获取专属端点：
   ```text
...
- **407 代理认证失败**：检查密码与用户名是否填错（填写在「账密与白名单」里设定的 Username/Password）。
- **连接超时**：确认 Host 和 Port 是否完全匹配提取页面给出的专有端口。
- **IP 遭到目标封禁**：在 **<a href="https://www.joyproxy.com/admin-my-orders.html" target="_blank" rel="noopener noreferrer">已购代理</a>** 中使用 **更换 IP** 刷新替换。
