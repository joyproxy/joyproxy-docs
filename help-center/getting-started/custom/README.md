...
如果你需要批量购买独享代理端口，且希望每个端口能灵活配置不同的地理位置或按设定频率自动更换出口 IP，自定义独享代理是最佳选择。如果你的业务固定只需要某一个国家且无需频繁更换 IP，请参考 [静态独享代理](../static/README.md)；如果需要按 GB 流量按需提取数百万池化 IP，请参考 [动态代理](../rotating/README.md)。
...
2. [网络类型与选型指南](network-types.md) — 住宅、商业/ISP 与数据中心自定义端口包对比
3. [购买端口](purchase.md) — 选择网络类型、端口数量与预分配选项下单
4. [查看与管理端口](view-ports.md) — 查看已购端口列表、生效状态与到期时间
5. [分配地区](assign-region.md) — 为端口分配或随时切换目标国家、省/州与城市
6. [设置定时轮换与手动更换 IP](rotation-and-refresh.md) — 配置自动轮换频率与手动换 IP
7. [自动续费](auto-renew.md) — 端口套餐余额自动扣费续订说明
8. [设置代理账密与白名单](authentication.md) — 配置代理用户名密码与 IP 白名单
9. [提取代理 IP](extract-ip.md) — 在控制台提取自定义端口端点列表及 API 引流
10. [发起代理请求](first-request.md) — Python、Node.js、Go、cURL 代码示例与第三方软件配置
11. [响应码与常见报错](response-codes.md) — 407、403、502 等常见报错快速排查
12. [受限目标与合规说明](restricted-targets.md) — 合规拦截与禁止访问范围说明
