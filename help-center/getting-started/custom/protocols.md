...
定制使用端点生成页的专用 `host:port`。支持 **HTTP** 与 **SOCKS5**，都能访问 **HTTPS 网站**。
...
`socks5h://` 由代理解析域名。白名单机器可以省略 `USER:PASS@`。

轮换在 `gate.joyproxy.com:9001` 使用相同协议，见 <a href="../rotating/protocols.md" target="_blank" rel="noopener noreferrer">轮换 → 协议</a>。
