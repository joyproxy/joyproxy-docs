...
5. 在 **预分配地区** 中可先指定部分端口国家，或选择 **购后再分配**。
6. 选择支付方式完成结算。
...
1. 进入 **<a href="https://www.joyproxy.com/admin-my-orders.html" target="_blank" rel="noopener noreferrer">已购代理</a>** 页面。
2. 在列表找到刚才购买的自定义代理端口。
3. 点击端口右侧的 **分配地区** 按钮。
...
1. 进入 **<a href="https://www.joyproxy.com/admin-authorization.html" target="_blank" rel="noopener noreferrer">账密与白名单</a>** 页面，在 **用户名 / 密码** 下新建代理账密（如 Username: `user123`，Password: `pass123`）。
2. 进入 **<a href="https://www.joyproxy.com/admin-ip-extraction-center.html" target="_blank" rel="noopener noreferrer">提取</a>** 页面，切换到 **自定义代理** 标签。
3. 选择协议（`HTTP` 或 `SOCKS5`），点击 **复制地址列表**，获取端口专属端点：
   ```text
...
- **407 代理认证失败**：检查用户名和密码是否填错（必须填在「账密与白名单」里设定的 Username/Password）；若采用白名单模式，检查本机公网 IPv4 是否已添加。
- **403 目标拒绝访问**：目标网站防护拦截了当前出口 IP。可在控制台中点击 **<a href="rotation-and-refresh.md" target="_blank" rel="noopener noreferrer">更换 IP</a>** 刷新出口。
- **502 / 504 连接超时**：检查端口是否已在 **<a href="view-ports.md" target="_blank" rel="noopener noreferrer">已购代理</a>** 中完成 [分配地区]。未分配地区的端口无法建立连接。
