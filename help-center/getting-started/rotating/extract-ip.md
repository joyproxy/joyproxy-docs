...
1. 在 **<a href="https://www.joyproxy.com/admin-ip-extraction-center.html" target="_blank" rel="noopener noreferrer">提取（Endpoint Generator）</a>** 页面点击 **API 提取（API Generator）** 页签。
2. 可选择 API 返回格式：**换行文本（Line Breaks CRLF）** 或 **JSON**。
3. 直接复制页面生成的 **API 地址（API URL）**。

> 💡 **OpenAPI 中心引流**  
> 详细的 API 请求路径、Query 参数（如 `token`, `country`, `duration` 等）以及完整代码调用示例，请直接查阅 **<a href="../../best-practices/openapi-center.md" target="_blank" rel="noopener noreferrer">OpenAPI 中心</a>** 中的 `GET /v2/extract` 接口文档。
...
- 在 **<a href="https://www.joyproxy.com/admin-ip-extraction-center.html" target="_blank" rel="noopener noreferrer">提取（Endpoint Generator）</a>** 页面点击 **AI 提取（AI Generator）** 了解详情。
- 详细指南请查阅 **<a href="../../best-practices/integrate-proxies-in-ai.md" target="_blank" rel="noopener noreferrer">在 AI 中集成代理</a>**。
