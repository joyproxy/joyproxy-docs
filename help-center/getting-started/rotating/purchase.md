...
Rotating is billed **per GB**. The pack stays usable until the traffic is consumed (and any validity shown at checkout still applies). Larger packs usually lower the unit price—compare on [Pricing](https://www.joyproxy.com/pricing.html) before you pay.

## Pick a network

| Network | Choose it when |
| --- | --- |
| **Residential** | You want home-ISP style exits and the widest geo menu |
| **Mobile** | You need carrier (4G/5G) exits for apps or mobile ads |
| **Business / ISP** | Targets expect an ISP / business ASN |

You can hold more than one network on the same account. Each order appears separately in [My Proxies](https://www.joyproxy.com/admin-my-orders.html).
...
4. Pay with **PayPal** or **account balance**. Top up on the same purchase page if the wallet is empty.
5. Wait until the order shows as active under [My Proxies](https://www.joyproxy.com/admin-my-orders.html).
