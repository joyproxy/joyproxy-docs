...
## 方式一：设置代理 Username/Password（必选）

这是连接动态代理网关唯一支持的鉴权方式。

1. 打开控制台 **<a href="https://www.joyproxy.com/admin-authorization.html" target="_blank" rel="noopener noreferrer">账密与白名单（Users &amp; Whitelist）</a>** 页面。
...
## 关于 IP 白名单（IP Whitelist）说明

> **重要提示**  
> 动态代理网关（`gate.joyproxy.com:9001`）**目前仅支持账密授权（Username/Password）**，不支持通过 IP 白名单免密访问。  
> 控制台中的 **IP 白名单** 功能主要用于静态独享代理免密连接，以及在固定公网服务器上免 Token 调用 API 提取代理接口。

如果你需要配置白名单用于静态代理或 API 提取：

1. 获取你客户端或爬虫服务器的**固定公网 IPv4 地址**。
2. 进入 **<a href="https://www.joyproxy.com/admin-authorization.html" target="_blank" rel="noopener noreferrer">账密与白名单（Users &amp; Whitelist）</a>** → **IP 白名单（IP Whitelist）** 页签。
