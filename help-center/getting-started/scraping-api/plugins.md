...
| --- | --- | --- | --- |
| **Amazon 亚马逊** | `/v1/fetch/plugin/amazon/pdp` | 抓取 Amazon 商品详情页（标题、价格、变体、评分、图片列表）。 | `asin=B08N5WRWNW&geocode=us` |
| **Amazon 亚马逊** | `/v1/fetch/plugin/amazon/search` | 抓取 Amazon 关键词搜索结果列表。 | `keyword=wireless+earbuds&geocode=us` |
| **Amazon 亚马逊** | `/v1/fetch/plugin/amazon/offer-listing` | 抓取 Amazon 商品跟卖报价与卖家列表。 | `asin=B08N5WRWNW&geocode=us` |
...
| **Walmart 沃尔玛** | `/v1/fetch/plugin/walmart/store` | 抓取 Walmart 商品与店铺基础信息。 | `item_id=12345678` |

---

## 调用方式与代码示例

插件 API 同样使用你的 **Web Unblocker Token（Scraping API Token）** 进行认证：
