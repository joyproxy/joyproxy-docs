...
JoyProxy 提供多种 AI 入口，不用手写样板 API 也能生成端点、查余额。
...
2. 在 OpenClaw 里配置 Skill 地址 `https://api.joyproxy.com/Skill`，填入 AI Access Token。
3. 用产品页示例语句生成轮换端点或查询余额。
...
3. 在 IDE 重启 MCP 客户端，调用工具生成线路或查询账户数据。

## 生产流量

AI 工具帮你**生成**连接字符串。实际 HTTP 流量仍按下面方式走：
...
打开 <a href="https://www.joyproxy.com/products/ai-customer-service.html" target="_blank" rel="noopener noreferrer">AI Assistant</a>，或在官网用悬浮聊天，可 24/7 获取指引。需要人工时通过<a href="https://www.joyproxy.com/admin-support.html" target="_blank" rel="noopener noreferrer">支持</a>升级。
